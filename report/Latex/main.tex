...
Section~1.1 applies gain compensation by dividing the rotated pair by floating-point $K$; only the quotient is re-quantized to $B$ fractional bits.
The constant $1/K$ itself is not quantized.

With ideal $K$, Table~\ref{tab:int-bits} shows that the main datapath fits in a \textbf{17-bit} word (worst case Q5.11).
No wider format is needed for matrix registers, CORDIC rotations, or iterative updates.

Hardware must approximate $1/K$ with a quantized constant.
Raising $B$ globally would widen the entire datapath even though the dynamic range analysis does not require it.

We therefore keep $B = 11$ on the datapath and introduce a separate fractional wordlength $B_K$ for $1/K$ only.
Each PE performs $X \times (1/K)$ with locally widened precision and truncates the product back to the datapath width before the result is passed on.
Extra bits are spent at this single multiply-and-round point, not across the systolic array.

Holding $B = 11$ and $N_{\mathrm{stage}} = 8$ fixed, we sweep $B_K$ independently.
All 11 patterns meet the RMSE criterion ($< 10^{-2}$) when $B_K \geq 16$ (Table~\ref{tab:k-config}).
...
\textbf{Parameter} & \textbf{Value} \\
\midrule
\texttt{B} (datapath fractional bits) & 11 \\
\texttt{B\_K} (fractional bits for $1/K$) & 16 \\
\bottomrule
\end{tabular}
\end{table}

% ─────────────────────────────────────────────
\chapter{Requirement 2: Bit-True Model Verification (11 Test Patterns)}
% ─────────────────────────────────────────────

\newpage

% ─────────────────────────────────────────────
\chapter{Requirement 3: Hardware Architecture and Control}
% ─────────────────────────────────────────────

\newpage

% ─────────────────────────────────────────────
\chapter{Requirement 4: Hardware Simulation}
% ─────────────────────────────────────────────

\newpage

% ─────────────────────────────────────────────
\chapter{Requirement 5: Fixed-Point vs.\ Hardware Consistency}
% ─────────────────────────────────────────────

\newpage

% ─────────────────────────────────────────────
\chapter{Requirement 6: Area-Time Product}
% ─────────────────────────────────────────────

\newpage

% ─────────────────────────────────────────────
\chapter{Requirement 7: Design Innovations and Team Contributions}
% ─────────────────────────────────────────────
